@{
    ...
    PrivateData = @{
        PSData = @{
            ...
            ReleaseNotes = 'v2.16.0: Export-ExecutiveSummary now surfaces the Security Maturity rating (via Get-GuerrillaMaturity): a color-coded Level X/5 badge in the stat row, plus a Security Maturity card with the overall level+label, the next-level blockers, and a per-category maturity table. This is the board-facing executive artifact that matches PingCastle maturity presentation and is stricter (worst-unmet-control anchors the rating). Report-only change, no new checks. Severity-color-coded L1 red to L5 green; empty-findings path safe. Remaining PingCastle items: report cartography (domain/trust/attack-path map) + full-domain transitive attack-path graph with BloodHound/AzureHound export. v2.15.0: New Get-GuerrillaMaturity cmdlet - a CMMI-style 1-5 security maturity rating from audit findings (AD / Google Workspace / Entra-M365). Worst unmet control anchors the score (one open Critical caps the estate at Level 1 Initial), which is stricter and more board-readable than an averaged 0-100 score. Levels 1 Initial -> 2 Managed -> 3 Defined -> 4 Quantitatively Managed -> 5 Optimized; FAIL caps by severity, any WARN caps at 4, PASS/SKIP/ERROR never cap. Returns overall level+label, per-category levels, the anchor findings holding you at the current level, and the next-level blockers; accepts pipeline input. First half of the executive-grade-artifact push (matches/exceeds PingCastle maturity). Next: report cartography + full-domain transitive attack-path graph with BloodHound export. 45 public functions. Test verify-maturity.ps1 (17/17). Check counts unchanged. See CHANGELOG.md for v2.15.0 and earlier.'
        }
    }
}

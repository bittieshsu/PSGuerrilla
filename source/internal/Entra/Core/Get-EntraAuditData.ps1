...
function Get-EntraAuditData {
    ...
    if (& $needsSource 'ModuleAvailability') {
        if (-not $Quiet) {
            Write-ProgressLine -Phase ENTRA -Message 'Checking module availability'
        }
    ...
    if (& $needsSource 'ConditionalAccess') {
        if (-not $Quiet) {
            Write-ProgressLine -Phase ENTRA -Message 'Collecting Conditional Access data'
        }
        try {
    ...
    if (& $needsSource 'AuthMethods') {
        if (-not $Quiet) {
            Write-ProgressLine -Phase ENTRA -Message 'Collecting authentication methods data'
        }
        try {
    ...
    if (& $needsSource 'PIM') {
        if (-not $Quiet) {
            Write-ProgressLine -Phase ENTRA -Message 'Collecting privileged identity data'
        }
        try {
    ...
    if (& $needsSource 'Applications') {
        if (-not $Quiet) {
            Write-ProgressLine -Phase ENTRA -Message 'Collecting application and service principal data'
        }
        try {
    ...
    if (& $needsSource 'Federation') {
        if (-not $Quiet) {
            Write-ProgressLine -Phase ENTRA -Message 'Collecting federation and hybrid identity data'
        }
        try {
    ...
    if (& $needsSource 'TenantConfig') {
        if (-not $Quiet) {
            Write-ProgressLine -Phase ENTRA -Message 'Collecting tenant configuration data'
        }
        try {
    ...
    if (& $needsSource 'AzureIAM') {
        if ($ArmAccessToken) {
            if (-not $Quiet) {
                Write-ProgressLine -Phase ENTRA -Message 'Collecting Azure IAM and resource security data'
            }
            try {
    ...
    if (& $needsSource 'Intune') {
        if (-not $Quiet) {
            Write-ProgressLine -Phase ENTRA -Message 'Collecting Intune endpoint management data'
        }
        try {
    ...
    if (& $needsSource 'M365Services') {
        if (-not $Quiet) {
            Write-ProgressLine -Phase ENTRA -Message 'Collecting M365 service configuration data'
        }
        try {

...
### Added
- **The desktop GUI now speaks 41 languages, four of them right-to-left.** Every user-visible string in Show-Guerrilla comes from locale catalogs in `Data/Locales/` (English is the source; each of the other 40 carries all 171 keys with per-key translation provenance). A header selector switches the whole window live, the choice persists in config, and first launch follows the OS display language when a matching catalog exists. Right-to-left languages (Arabic, Persian, Hebrew, Urdu) mirror the entire layout via a catalog-declared direction flag, while the run log and source inspector stay left-to-right because cmdlet output is English regardless of UI language. Adding a language is adding one catalog file: the selector discovers them at runtime, and codes may be script-qualified (Traditional Chinese ships as `zh-Hant` alongside Simplified `zh`). A localization gate (259 checks) fails the build if any string is missing from English, a shipped language is incomplete, a format placeholder is dropped or invented, or a declared text direction is invalid, and it proves its own scanner can fail with a poison self-test. The 40 translations ship as machine-draft pending native review; the per-key provenance tracks that. **Generated reports remain English only; this release localizes the application interface, not the HTML/PDF output.**
- **Ten more GUI languages, taking the toggle to 41.** Persian (RTL), Swahili, Amharic, Malay, Filipino, Romanian, Greek, Czech, Hungarian, and Traditional Chinese. Coverage now spans a Middle Eastern RTL, two major African languages (one in Ge'ez script), Southeast Asian (Malay distinct from Indonesian, Filipino), four more European languages including Greek's own script, and the Traditional/Simplified Chinese split (Taiwan conventions, verified distinct from the Simplified catalog). Persian is the fourth right-to-left language and reused the existing mechanism with no code changes. Traditional Chinese is the first catalog with a script-qualified BCP-47 code (`zh-Hant`); the loader, selector, config persistence, and gate are all string-based, so it flows through unchanged. The localization gate now covers all 41 languages (259 checks).
- **Twenty more GUI languages, taking the toggle to 31.** French, Italian, Hebrew (RTL), nine Indian languages (Bengali, Tamil, Telugu, Marathi, Gujarati, Kannada, Malayalam, Punjabi, and Urdu, which is RTL), plus Russian, Polish, Turkish, Vietnamese, Thai, Indonesian, Swedish, and Ukrainian. Each is a complete 171-key catalog with per-key `machine-draft` provenance, and Hebrew and Urdu reused the right-to-left mechanism built for Arabic with no code changes (catalog declares `"direction": "rtl"`, the window mirrors). No GUI code changed; twenty catalog files landed and the selector discovered them. The localization gate now covers all 31 languages (198 checks): completeness, no orphan keys, valid provenance, format-placeholder parity, and declared text direction. One cosmetic note left for human review: the optional secret-expiry date hint shows a localized mask in some languages (`AAAA-MM-DD`, `JJJJ-MM-TT`) and the literal `YYYY-MM-DD` in others; both are internally consistent and preserve year-month-day order, and the validator accepts digits regardless of the mask shown.
- **Four more GUI languages, including the first right-to-left one: Korean, Japanese, Simplified Chinese, and Arabic.** Eleven languages now ship. Arabic brought real RTL support to the architecture: a catalog declares its own text direction (`"_language": { ..., "direction": "rtl" }`), the loader surfaces it, and switching to an RTL language flips the whole window's `FlowDirection` so WPF mirrors the entire layout; the run log and the source inspector stay left-to-right because cmdlet output and PowerShell source are English regardless of UI language. The credential dialog mirrors to match. The localization gate validates each catalog's declared direction and that the loader reports it. Still no GUI code paths per language, just the shared RTL switch and four more catalogs.

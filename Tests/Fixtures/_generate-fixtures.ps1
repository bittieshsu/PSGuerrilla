...
# ─────────────────────────────────────────────────────────────────────────────
# ACTIVE DIRECTORY (Reconnaissance)  Test-Recon*
# ─────────────────────────────────────────────────────────────────────────────

# ADPRIV-001  Domain Admins membership count
New-Fixture AD ADPRIV-001 Reconnaissance clean PASS '2 effective members in Domain Admins is within the safe threshold (<=3)' @{
...
New-Fixture AD ADPRIV-001 Reconnaissance known-bad FAIL '6 effective members in Domain Admins exceeds the failing threshold (>5)' @{
    PrivilegedAccounts = @{ PrivilegedGroups = @{ 'Domain Admins' = @(
        @{ ObjectClass='user'; IsGroup=$false; SamAccountName='admin1'; Enabled=$true },
        @{ ObjectClass='user'; IsGroup=$false; SamAccountName='admin2'; Enabled=$true },
...
New-Fixture AD ADPRIV-001 Reconnaissance throttled SKIP 'PrivilegedMembers collection failed; must be Not Assessed, never PASS' @{
...
New-Fixture AD ADPRIV-022 Reconnaissance clean PASS 'krbtgt password age 90 days is within the safe threshold (<=180)' @{
    PrivilegedAccounts = @{ KrbtgtAccount = @{ PwdAgeDays = 90; KeyVersionNumber = 3 } }
    Errors = @{}
}
New-Fixture AD ADPRIV-022 Reconnaissance known-bad FAIL 'krbtgt password age 4000 days exceeds the failing threshold (>365)' @{
    PrivilegedAccounts = @{ KrbtgtAccount = @{ PwdAgeDays = 4000; KeyVersionNumber = 1 } }
    Errors = @{}
}
New-Fixture AD ADPRIV-022 Reconnaissance throttled SKIP 'PrivilegedMembers collection failed; must be Not Assessed, never PASS' @{
    PrivilegedAccounts = $null
    Errors = @{ PrivilegedMembers = 'krbtgt object query timed out.' }
}

# ADACL-002  GenericAll on critical objects
New-Fixture AD ADACL-002 Reconnaissance clean PASS 'no dangerous GenericAll ACEs present' @{
    ACLs = @{ DangerousACEs = @() }
    Errors = @{}
}
New-Fixture AD ADACL-002 Reconnaissance known-bad FAIL 'an unprivileged principal holds GenericAll over the domain root' @{
    ACLs = @{ DangerousACEs = @(
...
New-Fixture AD ADACL-002 Reconnaissance throttled SKIP 'ObjectACLs collection failed; must be Not Assessed, never PASS' @{
    ACLs = $null
    Errors = @{ ObjectACLs = 'LDAP ACL read failed: insufficient access rights.' }
}

# ADACL-015  Dangerous delegated rights
New-Fixture AD ADACL-015 Reconnaissance clean PASS 'no dangerous delegated ACEs after safe-admin filtering' @{
    ACLs = @{ DangerousACEs = @() }
    Errors = @{}
}
New-Fixture AD ADACL-015 Reconnaissance known-bad FAIL 'an unprivileged principal holds GenericWrite over the domain root' @{
    ACLs = @{ DangerousACEs = @(
...
New-Fixture AD ADACL-015 Reconnaissance throttled SKIP 'ObjectACLs collection failed; must be Not Assessed, never PASS' @{
    ACLs = $null
...
New-Fixture AD ADCS-002 Reconnaissance clean PASS 'no ESC1-vulnerable certificate templates' @{
...
New-Fixture AD ADCS-002 Reconnaissance known-bad FAIL 'a published template allows enrollee-supplied subject with client-auth EKU and low-priv enroll (ESC1)' @{
    CertificateServices = @{ CertificateTemplates = @(
...
New-Fixture AD ADCS-002 Reconnaissance throttled SKIP 'CertificateServices collection failed; must be Not Assessed, never PASS' @{
...
New-Fixture AD ADCS-010 Reconnaissance clean PASS 'no dangerous ACLs on CA enrollment-service objects' @{
    CertificateServices = @{
...
New-Fixture AD ADCS-010 Reconnaissance known-bad FAIL 'a low-priv principal holds WriteDacl over a CA enrollment-service object (ESC4-style)' @{
    CertificateServices = @{
...
New-Fixture AD ADCS-010 Reconnaissance throttled SKIP 'CertificateServices collection failed; must be Not Assessed, never PASS' @{
    CertificateServices = $null
...
New-Fixture AD ADKERB-004 Reconnaissance clean PASS 'no computer accounts with unconstrained delegation' @{
    Kerberos = @{ UnconstrainedDelegation = @() }
    Errors = @{}
}
New-Fixture AD ADKERB-004 Reconnaissance known-bad FAIL 'a workstation is trusted for unconstrained delegation' @{
    Kerberos = @{ UnconstrainedDelegation = @(
...
New-Fixture AD ADKERB-004 Reconnaissance throttled SKIP 'KerberosConfig collection failed; must be Not Assessed, never PASS' @{
    Kerberos = $null
    Errors = @{ KerberosConfig = 'Kerberos delegation query failed: domain controller unreachable.' }
}

# ─────────────────────────────────────────────────────────────────────────────
# ENTRA ID / M365 (Infiltration)  Test-Infiltration*
# ─────────────────────────────────────────────────────────────────────────────

# EIDPIM-001  Global Administrator count (substitute for broken EIDPIM-006)
New-Fixture Entra EIDPIM-001 Infiltration clean PASS '3 Global Administrators is within the recommended range (2-4)' @{
    PIM = @{ GlobalAdmins = @(
...
New-Fixture Entra EIDPIM-001 Infiltration known-bad FAIL '6 Global Administrators exceeds the failing threshold (>5)' @{
    PIM = @{ GlobalAdmins = @(
        @{ id='1'; displayName='Admin 1'; userPrincipalName='admin1@contoso.com'; userType='Member'; accountEnabled=$true },
        @{ id='2'; displayName='Admin 2'; userPrincipalName='admin2@contoso.com'; userType='Member'; accountEnabled=$true },
        @{ id='3'; displayName='Admin 3'; userPrincipalName='admin3@contoso.com'; userType='Member'; accountEnabled=$true },
...
New-Fixture Entra EIDPIM-001 Infiltration no-data SKIP 'Global Administrator list unavailable; must be Not Assessed, never PASS' @{
    PIM = @{ GlobalAdmins = $null }
    Errors = @{}
}

# EIDAUTH-002  MFA registration coverage
New-Fixture Entra EIDAUTH-002 Infiltration clean PASS '10 of 10 users MFA-registered (100%) meets the >=99% threshold' @{
    AuthMethods = @{ UserRegistrationDetails = @(
        1..10 | ForEach-Object { @{ id="$_"; isMfaRegistered=$true; isMfaCapable=$true } }
    ) }
    Errors = @{}
}
New-Fixture Entra EIDAUTH-002 Infiltration known-bad FAIL '0 of 10 users MFA-registered (0%) is below the 90% failing threshold' @{
    AuthMethods = @{ UserRegistrationDetails = @(
        1..10 | ForEach-Object { @{ id="$_"; isMfaRegistered=$false; isMfaCapable=$false } }
    ) }
    Errors = @{}
}
New-Fixture Entra EIDAUTH-002 Infiltration no-data SKIP 'User registration details unavailable; must be Not Assessed, never PASS' @{
    AuthMethods = @{ UserRegistrationDetails = @() }
    Errors = @{}
}

# EIDCA-007  Conditional Access universal MFA
New-Fixture Entra EIDCA-007 Infiltration clean PASS 'an enabled CA policy requires MFA for all users on all cloud apps' @{
...
New-Fixture Entra EIDCA-007 Infiltration known-bad FAIL 'no Conditional Access policy enforces MFA' @{
    ConditionalAccess = @{ Policies = @(
...
New-Fixture Entra EIDCA-007 Infiltration throttled SKIP 'ConditionalAccess collection failed; must be Not Assessed, never PASS' @{
...
New-Fixture Entra EIDAPP-005 Infiltration clean PASS 'a benign app (User.Read delegated only) with a credential-free service principal' @{
    Applications = @{
        AppRegistrations = @(
...
New-Fixture Entra EIDAPP-005 Infiltration known-bad FAIL 'an app with RoleManagement.ReadWrite.Directory holds a password credential' @{
    Applications = @{
        AppRegistrations = @(
...
New-Fixture Entra EIDAPP-005 Infiltration no-data SKIP 'Application/service-principal inventory unavailable; must be Not Assessed, never PASS' @{
    Applications = @{ AppRegistrations = $null; ServicePrincipals = $null }
    Errors = @{}
}

# EIDTNT-007  Security defaults / baseline protection
New-Fixture Entra EIDTNT-007 Infiltration clean PASS 'security defaults are enabled' @{
    TenantConfig = @{ SecurityDefaults = @{ isEnabled=$true } }
    ConditionalAccess = @{ Policies = @() }
    Errors = @{}
}
New-Fixture Entra EIDTNT-007 Infiltration known-bad FAIL 'security defaults disabled and no enabled CA policy replaces them' @{
    TenantConfig = @{ SecurityDefaults = @{ isEnabled=$false } }
    ConditionalAccess = @{ Policies = @( @{ state='disabled' } ) }
    Errors = @{}
}
New-Fixture Entra EIDTNT-007 Infiltration throttled SKIP 'TenantConfig collection failed; must be Not Assessed, never PASS' @{
...
# `RuleCount -eq 0 => PASS` branch in Test-InfiltrationM365EXO007 (latent bug).
New-Fixture Entra M365EXO-007 Infiltration clean PASS 'one benign disclaimer rule, no redirect/forward actions' @{
...
New-Fixture Entra M365EXO-007 Infiltration known-bad WARN 'an enabled transport rule redirects mail to an external address' @{
    M365Services = @{ Exchange = @{ TransportRules = @(
...
New-Fixture Entra M365EXO-007 Infiltration throttled SKIP 'Exchange transport-rule collection failed; must be Not Assessed, never PASS' @{
...
New-Fixture Entra M365AUDIT-001 Infiltration clean PASS 'unified audit log ingestion is enabled' @{
    M365Services = @{ AuditConfig = @{ UnifiedAuditLogIngestionEnabled=$true } }
    Errors = @{}
}
New-Fixture Entra M365AUDIT-001 Infiltration known-bad FAIL 'unified audit log ingestion is disabled' @{
    M365Services = @{ AuditConfig = @{ UnifiedAuditLogIngestionEnabled=$false } }
    Errors = @{}
}
New-Fixture Entra M365AUDIT-001 Infiltration throttled SKIP 'Audit configuration collection failed; must be Not Assessed, never PASS' @{
...
New-Fixture Entra INTUNE-010 Infiltration clean PASS 'an EDR configuration profile is deployed' @{
...
New-Fixture Entra INTUNE-010 Infiltration known-bad WARN 'no EDR configuration profile is deployed' @{
    Intune = @{ DeviceConfigurations = @(
        @{ id='c1'; displayName='Firewall Policy'; '@odata.type'='#microsoft.graph.windowsFirewallConfiguration' }
    ) }
    Errors = @{}
}
New-Fixture Entra INTUNE-010 Infiltration throttled SKIP 'Intune device-configuration collection failed; must be Not Assessed, never PASS' @{
    Intune = @{ DeviceConfigurations = $null }
    Errors = @{ Intune = 'Graph 429: deviceManagement/deviceConfigurations throttled.' }
}

# ─────────────────────────────────────────────────────────────────────────────
# GOOGLE WORKSPACE (Fortification)  Test-Fortification*
# ─────────────────────────────────────────────────────────────────────────────

# AUTH-001  2SV enforcement
New-Fixture GoogleWorkspace AUTH-001 Fortification clean PASS 'all active users have 2SV enforced (100% >= 95%)' @{
    Users = @(
...
New-Fixture GoogleWorkspace AUTH-001 Fortification known-bad FAIL '0% of active users have 2SV enforced (below 50%)' @{
    Users = @(
...
New-Fixture GoogleWorkspace AUTH-001 Fortification throttled SKIP 'User inventory collection failed; must be Not Assessed, never PASS' @{
    Users = $null
    Errors = @{ Users = 'Admin SDK 429: rate limit exceeded for users.list' }
}

# EMAIL-001  SPF hardening
New-Fixture GoogleWorkspace EMAIL-001 Fortification clean PASS 'SPF valid with a hard/soft-fail qualifier on every domain' @{
    DnsRecords = @{ 'example.com' = @{ SPF = @{ Valid=$true; Record='v=spf1 include:_spf.google.com ~all'; Details='' } } }
    Errors = @{}
}
New-Fixture GoogleWorkspace EMAIL-001 Fortification known-bad FAIL 'SPF ends in +all, allowing any host to send' @{
    DnsRecords = @{ 'example.com' = @{ SPF = @{ Valid=$true; Record='v=spf1 include:_spf.google.com +all'; Details='' } } }
    Errors = @{}
}
New-Fixture GoogleWorkspace EMAIL-001 Fortification throttled SKIP 'DNS record collection failed; must be Not Assessed, never PASS' @{
    DnsRecords = $null
    Errors = @{ 'DnsRecords:example.com' = 'DNS resolution failed: SERVFAIL' }
}

# EMAIL-003  DMARC enforcement
New-Fixture GoogleWorkspace EMAIL-003 Fortification clean PASS 'DMARC valid with an enforcing policy (reject)' @{
    DnsRecords = @{ 'example.com' = @{ DMARC = @{ Valid=$true; Policy='reject'; Details='' } } }
    Errors = @{}
}
New-Fixture GoogleWorkspace EMAIL-003 Fortification known-bad FAIL 'no valid DMARC record present' @{
    DnsRecords = @{ 'example.com' = @{ DMARC = @{ Valid=$false; Policy=$null; Details='DMARC record not found' } } }
    Errors = @{}
}
New-Fixture GoogleWorkspace EMAIL-003 Fortification throttled SKIP 'DNS record collection failed; must be Not Assessed, never PASS' @{
    DnsRecords = $null
    Errors = @{ 'DnsRecords:example.com' = 'DNS resolution failed: timeout' }
}

# OAUTH-008  Domain-wide delegation sensitive scopes
New-Fixture GoogleWorkspace OAUTH-008 Fortification clean PASS 'domain-wide delegation grants hold no Gmail/Drive/Admin/Calendar scopes' @{
    DomainWideDelegation = @(
        @{ clientId='100000000000000000001'; scopes=@('https://www.googleapis.com/auth/spreadsheets.readonly') }
    )
    Errors = @{}
}
New-Fixture GoogleWorkspace OAUTH-008 Fortification known-bad FAIL 'a delegation grant holds Gmail and Drive scopes' @{
    DomainWideDelegation = @(
        @{ clientId='100000000000000000002'; scopes=@('https://www.googleapis.com/auth/gmail.modify','https://www.googleapis.com/auth/drive') }
    )
    Errors = @{}
}
New-Fixture GoogleWorkspace OAUTH-008 Fortification throttled SKIP 'Domain-wide delegation collection failed; must be Not Assessed, never PASS' @{
...
New-Fixture GoogleWorkspace GTRADE-001 Fortification clean PASS 'delegation grants exist and hold only read-only scopes' @{
    DomainWideDelegation = @(
...
New-Fixture GoogleWorkspace GTRADE-001 Fortification known-bad FAIL 'a delegation grant holds full mailbox and directory write scopes' @{
    DomainWideDelegation = @(
        @{ clientId='100000000000000000004'; scopes=@('https://mail.google.com/','https://www.googleapis.com/auth/admin.directory.user') }
    )
    Errors = @{}
}
New-Fixture GoogleWorkspace GTRADE-001 Fortification throttled SKIP 'Domain-wide delegation collection failed; must be Not Assessed, never PASS' @{
    DomainWideDelegation = $null
    Errors = @{ DomainWideDelegation = 'Admin SDK: domain-wide delegation list returned 403' }
}
